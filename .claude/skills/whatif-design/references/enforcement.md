...
| Failures-as-data (no silent crashes) | All adapter and core code paths produce `FailureRecord` for expected failures, never raise unhandled exceptions to the CLI. Property test: synthetic adversarial inputs (malformed traces, scorer errors, network failures) all produce reports with non-empty `failures` array, no uncaught exceptions. CLI never exits 1 with an unhandled traceback for foreseeable failure modes. **Bug paths (not data paths)** raise `whatif.exceptions.InvariantViolationError` — a typed exception subclassing `Exception` directly (not `ValueError`/`RuntimeError`) so call-site intent is legible: structural type contract violated upstream, not a runtime data condition. Cardinal #1's "no silent crashes" promise covers expected failures; programmer-contract violations are bugs and propagate as typed errors with chained `__cause__`. See `src/whatif/exceptions.py` for the class docstring + boundary criteria. |
