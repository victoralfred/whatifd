...
- Conformance: ✅ `whatifd exec-check <target>` harness LANDED (drives hello → probe replay → shutdown, prints pass/fail per behavior, exit 0/2). Remaining: `examples/exec_agent_node/` (~60-line Node reference) and walkthrough fixture #8 (`regression_check` end-to-end over an exec runner, deterministic-subset byte-equality extending `tests/integration/test_determinism.py`).
