...
**Rippled to (spec-first DONE; loader+ExecRunner+teardown DONE; remaining rows in follow-up sub-PRs):**
- ✅ `src/whatifd/runner_loader.py` — recognizes `exec:`, shell-splits argv (POSIX rules), rejects when `os.name != "posix"`; returns a `LoadedRunner` wrapping a stateful `ExecRunner`. Loader error text names both schemes. (#151)
- ✅ New `src/whatifd/exec_runner.py` — `ExecRunner`: lazily spawns the child, runs the hello handshake, drives `replay_request → tool_lookup* → replay_response` per trace, maps the wire frame through the **same Pydantic models** as the Python lane (cardinal #5 unchanged). (#151) — NOTE: per-request `replay_seconds` timeout + child-restart is deferred to the manifest/restart sub-PR (the kernel applies the outer replay timeout today).
- ✅ **Subprocess lifetime vs the per-trace `Runner` protocol (load-bearing) — LANDED:** `_run_fork_pipeline` closes the runner in a `finally` around the `run_pipeline` call via `_close_runner` (best-effort, never masks the verdict). The child spawns lazily inside `run_pipeline`, so that `finally` is the complete teardown seam — no whole-function reindent. Stateless `python:` runners are a no-op.
- `failure_codes.py` — proposed new registry code `runner_protocol_error` (stage replay, scope trace, retryable false; `details`: `raw_excerpt` ≤256 chars, `violation`). Cardinal #1: protocol pathologies stay distinguishable from agent exceptions. DEFERRED until the replay layer maps `ExecRunnerError` onto failures (today `ExecRunner` raises and the kernel's boundary catch records `runner_exception`).
- `RunManifest`/report `runtime` — additive optional fields `runner_lane: "python"|"exec"` and `exec_runner: {argv, executable_sha256, runner_name, runner_version, protocol, restarts}`. `restarts` is non-deterministic; the rest `x-deterministic: true`. Additive but doctrine-guarded (cardinal #6) — schema doc + golden updates in the same PR. DEFERRED: would be dead schema until populated; lands with the manifest-population sub-PR.
...
- Conformance: ✅ `whatifd exec-check <target>` harness LANDED; ✅ `examples/exec_agent_node/agent.js` zero-dep Node reference runner LANDED (CI-validated via exec-check where node is present). ✅ exec end-to-end through the real pipeline COVERED (`test_cli_pipeline.py::test_exec_runner_runs_through_kernel_and_produces_score` — an `exec:` runner flows through `build_delta_fn` → replay kernel → scorer and its `ReplayOutput` reaches the `ScoreCase`). ✅ byte-equal determinism fixture #8 LANDED (`tests/integration/test_exec_determinism.py` — two fresh subprocess runs over an exec runner produce a byte-equal deterministic subset). The exec lane's conformance/integration coverage is now complete; only the deferred dead-schema items (`runner_protocol_error`, `runtime.exec_runner`) remain, intentionally unimplemented until populated.
