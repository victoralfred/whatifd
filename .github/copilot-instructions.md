...
- Planned source layout (per `.claude/skills/whatifd-design/phases.md`; not all dirs exist yet):
  ...
  - `src/whatifd/types/` — internal frozen-dataclass types (FailureRecord, DecisionFinding, CohortResult, FloorPassedProof, Sensitive[T], MethodologyDisclosure, etc.). Phase 1.
  - `src/whatifd/decision/` — floor evaluation, guard chain, finding/failure/fix-suggestion registries, cohort-systemic detection. Phase 2.
  - `src/whatifd/cache/` — scorer cache (keying, storage, lock, policy, summary). Phase 3.
  - `src/whatifd/adapters/` — adapter loader (lazy). Reference adapters live in separate packages: `whatifd-langfuse`, `whatifd-inspect-ai`. Phase 4.
  - `src/whatifd/report/` — public ReportV01 model + projection layer + JSON Schema. Phase 5.
  - `src/whatifd/serialization/` — encoder, graph walk for Sensitive[T], DecimalString, determinism subset. Phase 5.
  - `src/whatifd/replay/` — streaming pipeline (sync + async runners), tool cache, replay result types. Phase 6.
  - `src/whatifd/render/` — three-format renderer (1-line CI status, 30-line summary, full report). Phase 7.
  - `src/whatifd/cli.py`, `src/whatifd/config.py` — Typer CLI + Pydantic config. Phase 8.

- Where to look for design constraints: DESIGN.md, CONTRIBUTING.md, README.md, and `.claude/skills/whatifd-design/SKILL.md` (policy/doctrine for design decisions).
...
- Sensitive/defensive rules (from whatifd-design SKILL):
  ...
  - Trust floor cannot be bypassed; disclosure rules and two-affirmation apply (see `.claude/skills/whatifd-design/SKILL.md`).
...
- CLAUDE.md at repository root documents session telemetry and whatifd-design doctrine — read before making design/architecture proposals.
- `.claude/skills/whatifd-design/` contains SKILL.md and references that encode cardinal rules and session logging conventions. When using the whatifd-design skill, add session logs to `docs/sessions/` per the template.
