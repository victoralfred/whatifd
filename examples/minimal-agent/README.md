...
- Either a sync OR async runner — pick one; whatif doesn't mix them in a single run
...
- **Cardinal #5 (Sensitive at boundary):** the `TraceInput.user_message` your runner receives is a plain `str` — whatif unwraps the `Sensitive[str]` from the adapter side before handing it to you. Your `ReplayOutput.text` is also plain `str`; whatif rewraps for the report.
