"""`InspectAIScorer` — implementation of `whatif.adapters.Scorer`.

Wraps an Inspect AI scorer callable and produces `JudgeResult` +
`CacheKeyComponents` per the whatif `Scorer` protocol. The
runtime contract follows `whatif.adapters.protocols.Scorer`; the
...
def _resolve_package_version() -> str:
    ...
    at import time."""
    from whatif_inspect_ai import __version__
