...
## Why under `whatif.adapters` and not `tests/`
...
it under `whatif.adapters` makes it importable from anywhere
`whatif` is installed, while the lazy-load contract guarantees
that core never accidentally pulls it (`tests/unit/whatif/adapters/
