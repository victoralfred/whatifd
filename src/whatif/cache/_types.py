"""Shared types for `whatif.cache.*` — extracted to break the
serialization/lock circular import.

`whatif.cache.lock` and `whatif.serialization.lock_io` both need
...
`whatif.cache.lock` (the stable public surface), not from here.
"""
