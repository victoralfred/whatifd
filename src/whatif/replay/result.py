...
decision. `FailureRecord` (`whatif.types.failure`) is the
report-level shape — has stage/scope/id assigned, fits in
`ReportV01.failures`. The pipeline projects `ReplayFailure` →
`FailureRecord` via `whatif.decision.failure_codes.make_failure_record`
...
if TYPE_CHECKING:
    ...
    # `whatif.contract`. Type-only here to keep `whatif.replay`
    ...
    # discipline as `whatif.serialization.encoder` for `ReportV01`.
...
class ReplayFailure:
    ...
    - `tool_cache_miss` (raised from `whatif.replay.tool_cache`,
      Phase 6.2)
