...
### Behind the scene
1. Calls MCP server or AItool (e.g Claude/GPT) to write `src/whatif/adapters/<name>/__init__.py`
2. Patches `src/whatif/config.py` - adds fields to the datatype schema and loader
3. Patches `src/whatif/runner/factory.py` - adds the import and conditional `registry.register()`
