...
The `_ALLOWED_PACKAGE_PREFIX` is `whatif.serialization` (and its
...
_SRC_ROOT = Path(whatif.__file__).resolve().parent
_ALLOWED_PACKAGE_PREFIX = "whatif.serialization"


def _module_name_for(path: Path) -> str:
    """Compute the dotted module name for a file under `src/whatif/`."""
...
def test_no_json_dumps_outside_serialization_package() -> None:
    """Walk every `.py` file under `src/whatif/` and assert no
    `json.dumps` calls exist in modules outside `whatif.serialization.*`.
...
def test_module_name_resolution() -> None:
    ...
    assert _module_name_for(init_path) == "whatif.serialization"

    submodule = _SRC_ROOT / "serialization" / "encoder.py"
    assert _module_name_for(submodule) == "whatif.serialization.encoder"

    nested = _SRC_ROOT / "cache" / "keying" / "v1.py"
    assert _module_name_for(nested) == "whatif.cache.keying.v1"

    assert _is_serialization_module("whatif.serialization")
    assert _is_serialization_module("whatif.serialization.encoder")
    assert not _is_serialization_module("whatif.cache.keying.v1")
    assert not _is_serialization_module("whatif.serialization_helpers")  # prefix trap
