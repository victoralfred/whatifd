"""Tests for `whatifd.adapters.protocols` — Phase 4A.1.
...
6. `whatifd.adapters` is NOT imported by `import whatifd` (lazy-load
...
class TestReExports:
    def test_cluster_key_support_is_canonical_object(self) -> None:
        # The re-export from `whatifd.adapters` MUST be the same object
        # as the canonical home in `whatifd.types.statistical`. Catches
        ...
        from whatifd.adapters import ClusterKeySupport as Reexported
        from whatifd.types.statistical import ClusterKeySupport as Canonical
...
class TestLazyLoad:
    def test_import_whatif_does_not_load_adapters(self) -> None:
        ...
        # requires that `import whatifd` does not trigger
        # `whatifd.adapters` (or any concrete adapter module).
        result = subprocess.run(
            [
                sys.executable,
                "-c",
                "import whatifd, sys; "
                "loaded = [m for m in sys.modules if m.startswith('whatifd.adapters')]; "
                "print(loaded)",
            ],
            capture_output=True,
            text=True,
    ...
    def test_core_modules_do_not_load_adapters(self) -> None:
        # Strengthens the lazy-load contract: not just `import whatifd`,
        ...
        result = subprocess.run(
            [
                sys.executable,
                "-c",
                "import whatifd.cli, whatifd.diff, whatifd.config, whatifd.contract, "
                "whatifd.cache, whatifd.render, sys; "
                "loaded = sorted(m for m in sys.modules "
                "if m.startswith('whatifd.adapters')); "
                "print(loaded)",
            ],
            capture_output=True,
            text=True,
    ...
    def test_core_modules_do_not_load_real_adapter_packages(self) -> None:
        # Phase 4B contract: real adapter packages (`whatifd_langfuse`,
        # `whatifd_inspect_ai`) ship as separate distributions and MUST
        # NOT be imported by core. The previous test scans
        # `whatifd.adapters.*`; this one scans the sibling packages
        # directly. If a workspace install puts both on the same
        # `sys.path`, an accidental `import whatifd_langfuse` or
        # `import whatifd_inspect_ai` from core code would silently
