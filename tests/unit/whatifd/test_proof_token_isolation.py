...
module-private to `whatifd.config`. A re-export — accidental or
otherwise — would let any caller fabricate a proof and bypass the
two-affirmation check.

This module walks every `.py` under `src/whatifd/` and asserts:

  1. Outside `whatifd.config`, no module imports `_PROOF_TOKEN`.
  2. Inside `whatifd.config`, `_PROOF_TOKEN` does NOT appear in
     `__all__` (would re-export it via `from whatifd.config import *`).

Same lint pattern as the `json.dumps` ban
(`tests/unit/whatifd/serialization/test_banned_imports.py`):
AST walk, no extra dependencies, ~30 lines.

## What this lint does NOT cover

Runtime access via `getattr(whatifd.config, "_PROOF_TOKEN")` is
...
_SRC_ROOT = _REPO_ROOT / "src" / "whatifd"
_OWNER_MODULE = "whatifd.config"
...
def test_proof_token_not_imported_outside_config_module() -> None:
    ...
    assert not violations, (
        ...
        "inside whatifd.config or refactor the witness API."
    )
...
def test_proof_token_not_in_config_all() -> None:
    ...
    assert not _all_list_contains_token(tree), (
        ...
        "the token via `from whatifd.config import *` would let any "
...
def test_lint_self_check() -> None:
    ...
    # whatifd.config — without it the lint above would be vacuously
